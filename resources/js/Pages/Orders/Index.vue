    ...
    <Head title="Your Orders"/>
        <Authenticated>
            ...
            <input type="text" class="p-inputtext w-full lg:w-25rem surface-50" placeholder="Search"/>
                ...
                <span class="text-4xl" v-if="orders >= 0">No Orders to Display</span>
                <div v-for="order in orders" :key="id" class="surface-card grid grid-nogutter border-round shadow-2 mb-10">
                    <div class="col-12 flex p-2 surface-100 border-round-top">
                        ...
                        <div class="p-2 flex-auto text-center md:text-left">
                            ...
                            <span class="text-900 font-medium block mt-2">{{ order.created_at }}</span>
                        </div>
                        <Divider align="center" layout="vertical" class="h-full  mx-0 lg:mx-3 surface-border"></Divider>
                        <div class="p-2 flex-auto text-center md:text-left">
                            <span class="text-600 block">Total Amount</span>
                            <span class="text-900 font-medium block mt-2">${{ order.cost }}</span>
                        </div>
                    </div>
                    <div class="col-12">
                        <div class="p-2 my-4 flex flex-column lg:flex-row justify-content-between align-items-center">
                            <div class="flex flex-column lg:flex-row justify-content-center align-items-center px-2">
                                <img :src="order.product.media" alt="product" class="w-8rem h-8rem mr-3 flex-shrink-0" />
                                <div class="flex flex-column my-auto text-center md:text-left">
                                    <span class="text-900 font-medium mb-3 mt-3 lg:mt-0">{{ order.product.name }}</span>
                            ...
                            <Link v-ripple :href="route('orders.show', order.id)">
                                ...
                                <span class="bg-green-500 text-white flex align-items-center justify-content-center border-circle mr-2" style="min-width:2rem; min-height: 2rem">
                                    ...
                                    <span class="text-green-500">View Invoice</span>
                                </div>
                            </Link>
                        </div>
