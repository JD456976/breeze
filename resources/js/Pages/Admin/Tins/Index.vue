...
const deleteTin = (event, id) => {
    confirm.require({
        target: event.currentTarget,
        message: "Do you want to delete this tin?",
        icon: "pi pi-info-circle",
        acceptClass: "p-button-danger",
        accept: () => {
            Inertia.delete(route("admin.tins.destroy", id));
        },
        reject: () => {
            toast.add({
                severity: "info",
...
<template>
    <AdminLayout>
        <Head title="Tins List" />
        <div>
            <ConfirmPopup></ConfirmPopup>
            <div class="card">
                <Toast position="bottom-right" group="br" />
                <DataTable :value="tins" :paginator="true" class="p-datatable-customers" showGridlines :rows="10"
                    ...
                    <template #loading>
                        Loading tins data. Please wait.
                    </template>
                    <Column field="name" header="Name" style="min-width:12rem">
                        <template #body="{data}">
                            <Link v-tooltip.top="'Click To Edit'" :href="route('admin.tins.edit', data.id)">
                                {{data.name}}
                            </Link>
                        </template>
                    ...
                    <Column header="Delete">
                        <template #body="slotProps">
                            <Button
                                @click="deleteTin($event,slotProps.data.id)"
                                label="Delete"
                                v-tooltip.top="'Delete Tin'"
