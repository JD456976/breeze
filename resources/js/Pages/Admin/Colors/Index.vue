...
const deleteColor = (event, id) => {
    confirm.require({
        ...
        accept: () => {
            Inertia.delete(route("admin.colors.destroy", id));
        },
        reject: () => {
            toast.add({
                severity: "info",
    ...
    <AdminLayout>
        ...
        <div>
            ...
            <div class="card">
                ...
                <DataTable :value="colors" :paginator="true" class="p-datatable-customers" showGridlines :rows="10"
                    ...
                    <template #loading>
                        Loading colors data. Please wait.
                    </template>
                    <Column field="name" header="Name" style="min-width:12rem">
                        <template #body="{data}">
                            <Link v-tooltip.top="'Click to Edit'" :href="route('admin.colors.edit', data.id)">
                                {{data.name}}
                            </Link>
                        </template>
                    ...
                    <Column header="Delete">
                        <template #body="slotProps">
                            <Button
                                @click="deleteColor($event,slotProps.data.id)"
                                label="Delete"
                                v-tooltip.top="'Delete Color'"
